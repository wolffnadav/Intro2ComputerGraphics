...
public class MenuWindow extends JFrame implements Logger {
    ...
    public void removeObjectFromImage(boolean[][] srcMask) {
        log("removeObjectFromImage start...");
        BufferedImage result = duplicateImage();
        boolean[][] duplicateMask = duplicateMask(srcMask);

        int width = result.getWidth();
        while (true) {
            int getNewWidth = Math.min(countMaxNumberOfTrueValues(duplicateMask), ((width / 3) - 1));
            if (getNewWidth == 0) break;

            SeamsCarver sc = new SeamsCarver(this, result, (width - getNewWidth), this.colorMixer.getRGBWeights(), duplicateMask);
            result = sc.resize();
            duplicateMask = duplicateMask(sc.getMaskAfterSeamCarving());
            sc = new SeamsCarver(this, result, width, this.colorMixer.getRGBWeights(), duplicateMask);
            result = sc.resize();
            duplicateMask = duplicateMask(sc.getMaskAfterSeamCarving());
    ...
    private int countMaxNumberOfTrueValues(boolean[][] srcMask) {
        int max = 0;
        for (int y = 0; y < srcMask.length; y++) {
            int currMax = 0;
            for (int x = 0; x < srcMask[y].length; x++) {
                if (srcMask[y][x]) currMax++;
            }
